 ...
 exports.buildTermFilters = function (termOpts){
    ...
    for(var key in keys){
        if (Array.isArray(termOpts[key])) {
            // TODO: where clause is an array, treat as an $in query

        } else if (typeof termOpts[key] === 'object') {
            // TODO: where clause is an object - has the format $operator: value
        } else {
 ...
 * @return {Object}
 */
exports.buildFuzzyMatchingQuery = function (fuzzyOpts) {
    var queries = [];
...
exports.mergeSearchBody = function (searchOpts) {
    ...
    if(searchOpts.mustFuzzyMatch && Object.keys(searchOpts.mustFuzzyMatch).length){
        var mustFilters.push(exports.buildFuzzyMatchingQuery(searchOpts.mustFuzzyMatch));
    }

    //set shouldFuzzyMatch filters
    if(searchOpts.shouldFuzzyMatch && Object.keys(searchOpts.shouldFuzzyMatch).length){
        var shouldFilters.push(exports.buildFuzzyMatchingQuery(searchOpts.shouldFuzzyMatch));
    }

    //set must filters
    if(searchOpts.mustFilter && Object.keys(searchOpts.mustFilter).length){
        var mustFilters.push(exports.buildTermFilters(searchOpts.mustFilter));
    }
  
    //set should filters
    if(searchOpts.shouldFilter && Object.keys(searchOpts.shouldFilter).length){
        var shouldFilters.push(exports.buildTermFilters(searchOpts.shouldFilter));
    }

    //set must range filters
    if(searchOpts.mustRange && Object.keys(searchOpts.mustRange).length){
        var mustFilters.push(exports.buildRangeFilter(searchOpts.mustRange));
    }
  
    //set should range filters
    if(searchOpts.shouldRange && Object.keys(searchOpts.shouldRange).length){
        var shouldFilters.push(exports.buildRangeFilter(searchOpts.shouldRange));
    }
